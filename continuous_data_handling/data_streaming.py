...
from continuous_data_handling import config_event_analysis as cfg_event
from continuous_data_handling import signal_processing as proc
from continuous_data_handling import databases
import threading
...
class DataStream:
    ...
    def cut_stream(self, chans, samples, pad_zeros=True):
        ...
        if samples[0] > -1 and samples[1] < nt:
            ...
            overlap_chunk = c_stream.prev_chunk.cut(chans=(0, c_stream.curr_chunk.n_chan),
                                                    samples=(nt + samples[0], nt),
                                                    modify_stream=True)
    ...
    def __next__(self):
        ...
        if overlap_chunk.check_validity():
            ...
            if 'bandpass' in self.proc_steps:
                overlap_chunk.data = proc.bpfilter(overlap_chunk.data, dt=cfg.dt, bp_low=cfg.bp_low,
                                                   bp_high=cfg.bp_high)
            ...
            f_samp = int(self.internal_overlap / cfg.dt_decim_fac) - int(self.output_overlap / cfg.dt_decim_fac)
            l_samp = int(self.data.curr_chunk.nt / cfg.dt_decim_fac) + int(
                self.internal_overlap / cfg.dt_decim_fac) + int(self.output_overlap / cfg.dt_decim_fac)
            overlap_chunk.cut(chans=(0, overlap_chunk.n_chan), samples=(f_samp, l_samp), modify_stream=True)
